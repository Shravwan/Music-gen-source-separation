...
from main1.model_flute import FluteClassifier
from torchvision import transforms

# Constants
ROOT_DIR = r"D:\WSAI\data1\split\by_raga"
MODEL_PATH = r"D:\WSAI\main1\flute_model.pth"
SAMPLE_RATE = 22050
...
def extract_flute(audio_path, output_path):
    ...
    for i in range(chunks.size(0)):
        ...
        with torch.no_grad():
            ...
            if pred.item() > THRESHOLD:
                flute_segments.append(chunk.cpu().numpy())

    if flute_segments:
        output_audio = torch.cat([torch.tensor(seg) for seg in flute_segments], dim=-1)
        sf.write(output_path, output_audio.squeeze().numpy(), SAMPLE_RATE)
        print(f"Flute extracted: {output_path}")
    else:
        print(f"No flute detected in: {audio_path}")
...
for root, dirs, files in os.walk(ROOT_DIR):
    for file in files:
        if file.endswith("_other.wav"):
            ...
            flute_path = other_path.replace("_other.wav", "_flute.wav")

            if os.path.exists(flute_path):
                print(f"✅ Skipping {flute_path}, already exists.")
                continue

            print(f"🔍 Processing {other_path}...")
            extract_flute(other_path, flute_path)
